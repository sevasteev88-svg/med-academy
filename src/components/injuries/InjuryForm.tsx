  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await createInjury({
        ...
        munichGrade, bamicGrade, bamicLocation,
      });
    ...
    <div className="space-y-6">
      ...
      <section className="bg-surface rounded-xl border border-gray-800 p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Класифікація пошкодження</h3>
        ...
        <div className="flex gap-2">
          {(["none", "bamic", "munich"] as ClassificationSystem[]).map(s => (
            <button key={s} type="button"
              onClick={() => { setClassSystem(s); setMunichGrade(null); setBamicGrade(null); setBamicLocation(null); }}
              className={`px-3 py-1.5 text-xs rounded-lg border transition-colors ${classSystem === s ? "bg-brand-blue/20 border-brand-blue text-brand-blue font-medium" : "border-gray-700 text-gray-500 hover:border-gray-500 hover:text-gray-300"}`}>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={bamicGrade ?? ""} onChange={e => setBamicGrade(e.target.value === "" ? null : Number(e.target.value) as BamicGrade)}
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-blue">
                  <option value="">— оберіть —</option>
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">
                  Локалізація <span className="text-status-danger/70">(критично для RTP!)</span>
                </label>
                <select value={bamicLocation ?? ""} onChange={e => setBamicLocation(e.target.value === "" ? null : e.target.value as BamicLocation)}
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-blue">
                  <option value="">— оберіть —</option>
                  <option value="a">a — міофасціальне (краї)</option>
                  <option value="b">b — внутрішньом'язове (брюшко)</option>
                  <option value="c">c — T-junction ⚠️</option>
                </select>
              </div>
            </div>
            {bamicLocation === "c" && bamicGrade !== null && bamicGrade >= 2 && (
              <div className="flex gap-3 bg-status-danger/10 border border-status-danger/30 rounded-lg p-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-status-danger/80 mt-0.5">Термін реабілітації подвоєно. Обов'язковий МРТ-контроль.</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {MUNICH_GRADES.map(g => (
              <button key={g.value} type="button" onClick={() => setMunichGrade(g.value)}
                className={`text-left px-3 py-2.5 rounded-lg border text-xs transition-colors leading-tight ${munichGrade === g.value ? TYPE_SELECTED[g.type] : "border-gray-800 text-gray-500 hover:border-gray-600 hover:text-gray-300"}`}>
        ...
        {rtpPreview && (
          <div className={`rounded-lg border p-4 ${rtpPreview.is_t_junction_risk || rtpPreview.max_days >= 60 ? "bg-status-danger/10 border-status-danger/30" : rtpPreview.max_days >= 28 ? "bg-status-warn/10 border-status-warn/30" : "bg-status-ok/10 border-status-ok/30"}`}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-400">Прогноз RTP</p>
                <p className={`text-xl font-bold mt-0.5 ${rtpPreview.is_t_junction_risk || rtpPreview.max_days >= 60 ? "text-status-danger" : rtpPreview.max_days >= 28 ? "text-status-warn" : "text-status-ok"}`}>
                  {rtpPreview.min_days === rtpPreview.max_days ? `${rtpPreview.min_days} днів` : `${rtpPreview.min_days}–${rtpPreview.max_days} днів`}
                </p>
              </div>
              <div className="text-right text-xs text-gray-500">
                <p>Точність: {rtpPreview.confidence === "high" ? "висока" : "середня"}</p>
                {rtpPreview.is_t_junction_risk && <p className="text-status-danger font-medium mt-0.5">⚠️ T-junction</p>}
              </div>
            </div>
            {rtpPreview.notes && <p className="text-xs text-gray-400 mt-2 border-t border-gray-700/50 pt-2">{rtpPreview.notes}</p>}
          </div>
        )}
      </section>
