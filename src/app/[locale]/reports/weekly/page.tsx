...
import { INJURY_TYPE_UA, LOCATION_UA, SEVERITY_UA, STATUS_UA } from "@/lib/constants";
...
export default async function WeeklyReportPage() {
  ...
  const aiActiveInjuries = (activeInjuries ?? []).filter((i: any) => i.status === "active").map(mapInjury);
  const aiRehabInjuries = (activeInjuries ?? []).filter((i: any) => i.status === "rehabilitation").map(mapInjury);
  const allAiInjuries = [...aiActiveInjuries, ...aiRehabInjuries];
